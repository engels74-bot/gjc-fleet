...
`~/scripts/backuprestore/{backup-now.sh,restore.sh}` — snapshot + full-revert tooling; the runbook
states every Phase-G/relay artifact is registered for teardown (`restore.sh --apply`, optional
...
- Whether `~/scripts/backuprestore/` snapshots include the relay stack added after Phase G
  (the runbook asserts registration, not re-verified here).
